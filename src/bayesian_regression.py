# =================================================================================================================================== #
# ----------------------------------------------------------- DESCRIPTION ----------------------------------------------------------- #
# This script contains the necessary classes and functions to conduct the Bayesian Regression layer in the AoA Estimator.             #  
# =================================================================================================================================== #
...
import seaborn                 as sns                      # Seaborn for enhanced data visualization aesthetics.                      #
...
from   sklearn.preprocessing   import StandardScaler       # Standardization of features by removing the mean and scaling to unit var.#
...
from   pyro.nn                 import PyroModule           # PyroModule for creating probabilistic models.                            #
from   pyro.nn                 import PyroSample           # PyroModule for creating probabilistic models.                            #
from   pyro.infer              import SVI                  # Stochastic Variational Inference (SVI) for training models.              #
from   pyro.infer              import Trace_ELBO           # Stochastic Variational Inference (SVI) for training models.              #
from   pyro.infer              import Predictive           # Stochastic Variational Inference (SVI) for training models.              #
...
pyro.set_rng_seed(42) # Set Pyro random seed for reproducibility.                                                                     #
torch.manual_seed(42) # Set PyTorch random seed for reproducibility.                                                                  #
...
class BayesianAoARegressor:
    ...
    def _extract_features(self, data_manager, include_distance=True, include_width=True):
        ...
        Parameters:
            - data_manager       [DataManager] : The DataManager object containing the dataset
            - include_distance   [bool]        : Whether to include distance feature
            - include_width      [bool]        : Whether to include width feature

        Returns:
            - X                  [np.ndarray]  : Feature matrix
            - y                  [np.ndarray]  : True angles
            - feature_names      [list]        : List of feature names
            - prior_estimates    [dict]        : Dictionary of prior estimates from physics-based methods
        """
    ...
    def _build_model(self, input_dim, prior_mean, prior_std):
        ...
        Parameters:
            ...
            - prior_mean   [float] : Mean for the prior distribution
            - prior_std    [float] : Standard deviation for the prior distribution

        Returns:
            - model        [PyroModule]: The constructed Bayesian linear regression model
        """
    ...
    def train(self, data_manager, num_epochs=1000, learning_rate=0.01):
        ...
        Returns:
            - train_summary [dict]        : Summary of training results including metrics and predictions
        """
        ...
        if self.prior_type in prior_estimates:
            # Calculate the standard deviation of the difference between the physics-based estimate and the true angle
    ...
    def predict(self, X, return_uncertainty=True):
        ...
        Returns:
            ...
            - y_pred_std         [np.ndarray] : Standard deviation of predictions (if return_uncertainty=True)
        """
    ...
    def _get_display_prior_name(self):
        """
        Return a display-friendly name for the prior type.
        
        Returns:
            - str: Display name for the prior type
        """
        prior_display_names = {
            'ds': 'DS Beamforming',
        ...
        return prior_display_names.get(self.prior_type, self.prior_type.upper())
    ...
    def visualize_results(self, output_dir, experiment_name):
        ...
        prior_colors = {'ds': 'g', 'weighted': 'm', 'music': 'c', 'phase': 'y', 'flat': 'y', 'pd': 'y'}
        prior_markers = {'ds': '^', 'weighted': 's', 'music': 'd', 'phase': 'x', 'flat': 'x', 'pd': 'x'}
        prior_labels = {
            'ds': 'DS Beamforming', 
            'weighted': 'Weighted DS', 
            'music': 'MUSIC', 
        ...
        for key in prior_test:
            if self.prior_type == key:
                ...
                plt.scatter(y_test, prior_test[key], marker=prior_markers[key], 
                          color=prior_colors[key], alpha=0.5, s=120,
                          label=f"{prior_labels[key]} (Prior)")
            else:
                plt.scatter(y_test, prior_test[key], marker=prior_markers[key], 
                          color=prior_colors[key], alpha=0.3, s=80,
                          label=f"{prior_labels[key]}")
        plt.xlabel('True Angle (degrees)')
        plt.ylabel('Predicted Angle (degrees)')
        plt.title(f'Bayesian AoA Model with {self._get_display_prior_name()} Prior\n'
    ...
    def visualize_prior_vs_posterior(self, output_dir, experiment_name, num_samples=5):
        ...
        for i, idx in enumerate(indices):
            ...
            ax.plot(x, prior_pdf, 'r--', linewidth=2, label=f'{self._get_display_prior_name()} Prior')
    ...
    def _plot_feature_group(self, indices, group_name, prior_mean, prior_std, weight_loc, weight_scale, vis_dir):
        ...
        Parameters:
            - indices        [list] : List of feature indices to plot
            - group_name     [str]  : Name of the feature group (e.g., "Phase-Related")
            - prior_mean     [float]: Mean of the prior distribution
            - prior_std      [float]: Standard deviation of the prior distribution
            - weight_loc     [np.ndarray]: Posterior mean weights
            - weight_scale   [np.ndarray]: Posterior standard deviations of weights
    ...
    def analyze_with_posterior_weights(self, data_manager, output_dir, experiment_name):
        ...
        Parameters:
            - data_manager    [DataManager] : DataManager instance with signal data and metadata
    ...
    def _extract_features_for_sample(self, phasor1, phasor2, rssi1, rssi2, D, W, wavelength):
        ...
        Parameters:
            - phasor1       [np.ndarray] : Phasor data for sensor 1
            - phasor2       [np.ndarray] : Phasor data for sensor 2
            - rssi1         [np.ndarray] : RSSI data for sensor 1
            - rssi2         [np.ndarray] : RSSI data for sensor 2
...
def train_bayesian_models(data_manager, results_dir, num_epochs=10000):
    ...
    configs = [
        ...
        {"prior": "phase", "features": "full", "name": "phase_full"},  # Changed from flat to phase
        ...
        {"prior": "phase", "features": "width_only", "name": "phase_width"},  # Changed from flat to phase
        ...
        {"prior": "phase", "features": "sensor_only", "name": "phase_sensor"}  # Changed from flat to phase
    ]
...
def compare_bayesian_models(models, results, output_dir):
    ...
    Parameters:
        - models       [dict] : Dictionary of trained BayesianAoARegressor models
        - results      [dict] : Dictionary of training results for each model
    ...
    for name in sorted_names:
        parts = name.split('_')
        # Convert prior name to display name
        if parts[0].upper() == "FLAT" or parts[0].upper() == "PHASE":
            prior = "PD"
        elif parts[0].upper() == "WEIGHTED":
            prior = "Weighted DS"
        elif parts[0].upper() == "DS":
            prior = "DS"
        else:
            prior = parts[0].upper()
        if parts[1] == 'full':
    ...
    for feat_mode in feature_modes:
        ...
        if len(models_with_feat) > 1:
            ...
            for i in range(len(models_with_feat), n_rows * n_cols):
                row_idx = i // n_cols
                col_idx = i % n_cols
